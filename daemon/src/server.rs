...
fn guardian_details(daemon: &Daemon, id: &str, body: &str) -> Reply {
    ...
    // in one of these statuses must therefore always go through
    ...
    // `merge_failed`). `finalizing` is the same in-flight window (RAL-422),
    // so a details edit there restarts the merge for the same reason.
    if matches!(
        status.as_str(),
