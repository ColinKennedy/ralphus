...
impl Store {
    ...
    pub fn list_agent_profiles(&self) -> StoreResult<Vec<AgentProfileView>> {
        let mut stmt = self.conn.prepare(
            "SELECT name, backend, executable, model, env_json, created_at_ms, updated_at_ms
    ...
    pub fn upsert_agent_profile(
        ...
        let now = now_ms();
        self.conn.execute(
            "INSERT INTO agent_profiles(name, backend, executable, model, env_json, created_at_ms, updated_at_ms)
             VALUES(?,?,?,?,?,?,?)
            ...
            rusqlite::params![name, backend, executable, model, env_json, now, now],
        )?;
        crate::rlog!(
            INFO,
...
mod tests {
    ...
    fn upsert_overwrites_an_existing_profile_by_name() {
        let store = Store::open_in_memory().unwrap();
        store
            .upsert_agent_profile("p", "pi", None, None, vec![set("A", "1")])
            .unwrap();
        store
            .upsert_agent_profile("p", "codex", None, Some("gpt-6"), vec![set("A", "2")])
            .unwrap();
    ...
    fn upsert_rejects_a_reserved_name() {
        let store = Store::open_in_memory().unwrap();
        let err = store
            .upsert_agent_profile("claude-code", "pi", None, None, vec![])
            .unwrap_err();
    ...
    fn upsert_rejects_an_unknown_backend() {
        let store = Store::open_in_memory().unwrap();
        let err = store
            .upsert_agent_profile("p", "not-a-backend", None, None, vec![])
            .unwrap_err();
    ...
    fn upsert_rejects_executable_on_a_native_backend() {
        let store = Store::open_in_memory().unwrap();
        let err = store
            .upsert_agent_profile("p", "claude", Some("/usr/bin/claude"), None, vec![])
            .unwrap_err();
        assert!(matches!(
            err,
    ...
    fn upsert_rejects_executable_on_an_overridable_backend() {
        let store = Store::open_in_memory().unwrap();
        let err = store
            .upsert_agent_profile("p", "claude-code", Some("/usr/bin/claude"), None, vec![])
            .unwrap_err();
        assert!(matches!(
            err,
    ...
    fn upsert_requires_executable_on_the_raw_backend() {
        let store = Store::open_in_memory().unwrap();
        let err = store
            .upsert_agent_profile("p", "raw", None, None, vec![])
            .unwrap_err();
        assert!(matches!(
            err,
    ...
    fn upsert_accepts_an_executable_on_the_raw_backend() {
        let store = Store::open_in_memory().unwrap();
        store
            .upsert_agent_profile("p", "raw", Some("/usr/bin/my-tool"), None, vec![])
            .unwrap();
        let profile = store.get_agent_profile("p").unwrap().unwrap();
    ...
    fn list_is_alphabetical_and_delete_reports_whether_a_row_existed() {
        let store = Store::open_in_memory().unwrap();
        store
            .upsert_agent_profile("zeta", "pi", None, None, vec![])
            .unwrap();
        store
            .upsert_agent_profile("alpha", "pi", None, None, vec![])
            .unwrap();
    ...
    fn list_for_backend_filters_by_backend() {
        let store = Store::open_in_memory().unwrap();
        store
            .upsert_agent_profile("a", "claude-code", None, None, vec![])
            .unwrap();
        store
            .upsert_agent_profile("b", "pi", None, None, vec![])
            .unwrap();
        store
            .upsert_agent_profile("c", "claude-code", None, None, vec![])
            .unwrap();
        let names: Vec<String> = store
    ...
    fn references_reports_no_referrers_for_an_unused_profile() {
        let store = Store::open_in_memory().unwrap();
        store
            .upsert_agent_profile("unused", "pi", None, None, vec![])
            .unwrap();
    ...
    fn references_finds_cells_tasks_and_proofs_by_squad() {
        let store = Store::open_in_memory().unwrap();
        store
            .upsert_agent_profile("my-agent", "pi", None, None, vec![])
            .unwrap();
        store
            .conn
            .execute(
    ...
    fn references_finds_guardians_by_any_agent_role_column() {
        let store = Store::open_in_memory().unwrap();
        store
            .upsert_agent_profile("resolver-agent", "pi", None, None, vec![])
            .unwrap();
        store
            .conn
            .execute(
