...
//! excerpt, and asking whether to fix it as review feedback -- the exact
//! phrase downstream routing keys on. A terminal success is silent.
//!
...
/// text), and a closing question that must contain the literal phrase
/// "review feedback" -- a required keyword used downstream to route the
/// user's reply, not just conversational wording. Wiring up that reply is
/// out of scope here; only the question needs to exist.
fn enqueue_ci_failure_notice(
    ...
    text.push_str("\nWould you like to fix this as review feedback?");
