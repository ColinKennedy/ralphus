...
/// candidates (RAL-412) exactly like the cron-drain path: the firing call
/// funnels into [`build_review_from_drained_pool`], which hands every
/// candidate's bounded prompt context to one aggregate Arbiter request
/// ([`arbiter_pool_order`]) and applies the validated proposed order to the
/// branch stack; `orderer` returning `None` leaves the deterministic pool
/// order. Production callers pass the configured-Arbiter request
/// (`arbiter_pool_order`); tests substitute a canned closure.
///
...
/// until enough cells have actually completed successfully. Each fired
/// pool's review asks `orderer` for a semantic order of its candidates
/// (RAL-412), exactly like the submit-time path: see
/// [`create_review_from_triage_pool`] (`orderer` returning `None` leaves
/// the deterministic pool order). Production callers pass the
...
/// otherwise the deterministic pool order. Production callers pass the
/// configured-Arbiter request ([`arbiter_pool_order`]); tests substitute a
/// canned closure.
///
...
///
/// The combined review asks `orderer` for a semantic order of its
/// candidates (RAL-412) exactly like [`create_review_from_triage_pool`]:
/// one aggregate Arbiter request over every candidate's bounded prompt
/// context, applied when the reply is a valid permutation, otherwise the
...
pub fn repair_triage_pool_keys(store: &Store) {
    ...
    for (project, triage_type) in touched_keys {
        ...
        if threshold.is_some_and(|t| count >= t) {
            match create_review_from_triage_pool(
                store,
                &project,
                &triage_type,
...
"#
    ...
    fn create_review_from_triage_pool_applies_the_proposed_semantic_order_to_the_branch_stack() {
        let mut store = Store::open_in_memory().unwrap();
        // A candidate must have completed before it can form part of a
        // review (RAL-318: pending cells never count toward a firing
        // threshold), so each pooled cell is backed by a done squad.
        let squad_1 = completed_pool_cell(&mut store);
        let squad_2 = completed_pool_cell(&mut store);
    ...
    /// RAL-412: the whole-pool drain hands the Arbiter every
    ...
    fn build_review_from_drained_pool_carries_each_candidates_prompt_context() {
        ...
        let squad_id = store.insert_squad(&file, None, false).unwrap();
        // RAL-318: only a completed candidate is viable for a review.
        store
            .set_cell_state(&squad_id, 0, 0, crate::store::NodeState::Done)
            .unwrap();
        store
    ...
    fn derive_triage_pools_pools_a_local_cell_and_fires_on_threshold() {
        ...
        store
            ...
            .unwrap();
        let created = fire_ready_triage_thresholds(&store, &squad_id_2, |_| None).unwrap();
    ...
    fn triage_pool_worktree_sibling_gates_review_ready_until_both_cells_done() {
        ...
        store
            .set_cell_state(&squad_id, 0, 0, crate::store::NodeState::Done)
            .unwrap();
        let created = fire_ready_triage_thresholds(&store, &squad_id, |_| None).unwrap();
