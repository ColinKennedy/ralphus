// Loads the resolver-agent <select>'s option-building + lazy-load logic
// (RAL-444) out of the board chunk files (librarian/assets/board/) so it can
...
// the region between the RALPHUS-RESOLVER-AGENT-SELECT markers calls only
// `esc` and the ambient `fetch`/`setTimeout` globals (the latter two stubbed
// per-test via `globalThis.fetch` / node:test's mock timers) plus a shared
...
const BEGIN = "// RALPHUS-RESOLVER-AGENT-SELECT:BEGIN";
const END = "// RALPHUS-RESOLVER-AGENT-SELECT:END";
...
if (from === -1 || to === -1 || to < from) {
  throw new Error(
    `board-resolver-agent-select: could not find the ${BEGIN} / ${END} markers in the board chunks. ` +
...
const factory = new Function(
  ...
  `${source}
   return {
     resolverOptionHtml,
     resolverOptionHtmlFromEntry,
     ensureAgentOptionsLoaded,
     onResolverSelectMouseDown,
     RESOLVER_AGENT_FALLBACK_AGENTS,
     RESOLVER_AGENT_FALLBACK_DEFAULT,
     RESOLVER_AGENT_LOAD_TIMEOUT_MS,
   };`,
);
...
/**
 * Builds the resolver-agent select helpers, evaluated straight from the
 * board chunks, wired to a fresh `agentOptionsByCwd` cache (or one supplied
 * by the caller, to share state across calls the way the real page's
 * page-lifetime cache does) and a `renderReviewDetail` spy.
 ...
 */
export function makeResolverAgentSelect(deps = {}) {
  const {
