...
`squash_review_commits` (`daemon/src/guardian_merge.rs:10402`) commits the
squashed result with `--no-verify`, which skips the `prepare-commit-msg` hook
that RAL-445 installs to append `Co-authored-by:` (and any future
`Ralphus-Cell:`) trailers. It also rebuilds the squashed commit message from
`git log --format=%s` — subjects only — so even trailers that *did* make it
onto the individual squashed commits are dropped from the message being
rebuilt.
