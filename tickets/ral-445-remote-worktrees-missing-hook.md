...
`sync_coauthor_hook` (`daemon/src/git_hooks.rs:98`) is only called from the
local `git worktree add` path — `execute_worktree_plan` in
`daemon/src/worktrees.rs`, via `sync_coauthor_hook_best_effort` at line 1182.
`provision_remote_with_targets` (`daemon/src/worktrees.rs:1894`) never calls
it.
