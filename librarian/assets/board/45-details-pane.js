      ...
      /**
       * One row of a four-state (inherited/overridden/added/unset)
       * env-override table -- shared by every scope where a layer doesn't
       * own its variables outright but shadows an *inherited* baseline
       * instead (RAL-191's review branches, RAL-203's combined-worktree
       * build/manual-checks steps):
       *   - inherited  — value comes from the baseline (italic value, per
       *                  docs/colors.md's "inherited resolved value" rule)
       *   - overridden — this layer replaced the inherited value
       *   - added      — this layer set a variable the baseline never had
       *   - unset      — this layer tombstoned it; the step does not get it
       *                  at all (struck through, muted)
       * @param {string} key
       * @param {{[key: string]: string}} inherited
       * @param {{[key: string]: (string|null)}} own
       * @param {string} apiPath
       * @param {string} scopeLabel - human-readable scope name for prompt/confirm copy, e.g. "this review worktree" or "the build step".
       * @returns {string}
       */
      function renderInheritedEnvOverrideRow(key, inherited, own, apiPath, scopeLabel) {
        const hasOwn = Object.prototype.hasOwnProperty.call(own, key);
        const ownVal = hasOwn ? own[key] : undefined;
        const isUnset = hasOwn && ownVal === null;
        const isInherited = !hasOwn;
        const shown = isUnset ? (inherited[key] ?? "") : (hasOwn ? String(ownVal) : inherited[key]);
        const tag = isUnset
          ? `<span class="badge" style="color:var(--muted);border-color:var(--muted);font-size:10px" data-tip="Tombstoned here — removed from ${esc(scopeLabel)}'s environment entirely, even though it would otherwise be inherited.">unset</span>`
          : isInherited
            ? `<span class="badge" style="color:var(--muted);border-color:var(--border);font-size:10px" data-tip="Inherited — change it upstream (or override it here) to affect ${esc(scopeLabel)}.">inherited</span>`
            : (key in inherited)
              ? `<span class="badge" style="color:var(--accent);border-color:var(--accent);font-size:10px" data-tip="Overridden here — replaces the inherited value for ${esc(scopeLabel)} only.">overridden</span>`
              : `<span class="badge" style="color:var(--accent);border-color:var(--accent);font-size:10px" data-tip="Added here — not present upstream.">added</span>`;
        // docs/colors.md: an inherited resolved value renders italic in the
        // normal text color — provenance, not a status or a warning.
        const valStyle = isUnset
          ? "flex:1;text-decoration:line-through;color:var(--muted)"
          : isInherited
            ? "flex:1;font-style:italic"
            : "flex:1";
        const revertBtn = hasOwn
          ? `<button class="btn" data-click="clearScopedEnvOverride" data-api-path="${esc(apiPath)}" data-key="${esc(key)}" data-scope-label="${esc(scopeLabel)}" data-tip="Drop this setting for ${esc(key)}, so it goes back to inheriting the upstream value.\nWho/when: you overrode or unset a variable for ${esc(scopeLabel)} and now want the inherited value back.\nThis cannot be undone.">↩ Inherit</button>`
          : "";
        const unsetBtn = isUnset
          ? ""
          : `<button class="btn" data-click="unsetScopedEnvOverride" data-api-path="${esc(apiPath)}" data-key="${esc(key)}" data-scope-label="${esc(scopeLabel)}" data-tip="Remove ${esc(key)} from ${esc(scopeLabel)}'s environment entirely, even though it would otherwise be inherited.\nWho/when: an upstream variable (a token, a proxy) that this step must run without.\nThis cannot be undone.">⊘ Unset</button>`;
        return `<div class="kv-row" style="margin:0">
            <span class="v mono" style="flex:1;font-size:11px">${esc(key)}</span>
            <span class="v mono" style="${valStyle};font-size:11px">${esc(shown)}</span>
            ${tag}
            <button class="btn" data-click="setScopedEnvOverride" data-api-path="${esc(apiPath)}" data-key="${esc(key)}" data-scope-label="${esc(scopeLabel)}" data-tip="Set a value for ${esc(key)} here, replacing the inherited one for ${esc(scopeLabel)} only.">✎ Override</button>
            ${unsetBtn}${revertBtn}
          </div>`;
      }
      /**
       * Renders the per-review-worktree "environment overrides" section shown
       * inside one review branch's expanded row (RAL-191).
       *
       * Unlike every other scope's widget (`renderEnvOverrides`), a review
       * branch does not own its variables to begin with: it inherits its
       * source cell's resolved environment, because the worktree is built
       * from that cell's work — see `renderInheritedEnvOverrideRow` for the
       * four-state row logic this (and RAL-203's build/manual-checks
       * sections) share.
       * @param {GuardianView} g
       * @param {GuardianBranch} b
       * @returns {string}
       */
      function renderBranchEnvOverrides(g, b) {
        const inherited = b.inherited_env || {};
        const own = b.env_overrides || {};
        const apiPath = `/api/guardians/${g.id}/branches/${b.id}/env`;
        const keys = Array.from(new Set(Object.keys(inherited).concat(Object.keys(own)))).sort();
        const overrideCount = Object.keys(own).length;
        const banner = overrideCount
          ? `<div class="kv-row" style="margin:0 0 2px" data-tip="This review worktree does not run with exactly the environment its source cell ran under.\nThe conflict resolver, final-proof pass, feedback routing, and check gates for this branch all use the values shown below.">⚠ ${overrideCount} branch override${overrideCount === 1 ? "" : "s"} — this worktree differs from its source cell</div>`
          : "";
        const rows = keys.length
          ? keys.map((k) => renderInheritedEnvOverrideRow(k, inherited, own, apiPath, "this review worktree")).join("")
          : `<div class="kv-row" style="margin:0"><span class="v">None</span></div>`;
        return `<h3 class="section" style="margin-top:8px" data-tip="The environment this branch's review worktree runs under.\nA review worktree is assembled from a cell's work, so it inherits that cell's resolved environment by default — the conflict resolver needs the same variables the code was written under.\nOverride or unset any of them here to change just this branch's review, leaving the original task untouched.">environment overrides (review worktree)</h3>
          ${banner}
          ${rows}
          <div class="kv-row" style="margin:2px 0 0">${envViewerBtn(apiPath, "this review worktree")}<button class="btn" data-click="addEnvOverrideAt" data-api-path="${esc(apiPath)}" data-tip="Add an environment variable for this review worktree only.\nWho/when: the review needs a variable its source cell never set — a reviewer-only flag, a different API base, a test credential.\nApplies the next time this branch's resolver, proof pass, feedback routing, or check gates run, and stays set until removed.">+ Add override</button></div>`;
      }
      /**
       * Renders the build/check-gate step's "environment overrides" section
       * (RAL-203), shown in a review's "check gates" area. The combined
       * worktree has no source cell of its own to inherit from, so this
       * shadows `g.combined_env` — the last enabled branch's own resolved
       * environment — instead. Independent of the manual-checks layer
       * rendered by `renderGuardianManualChecksEnvOverrides`.
       * @param {GuardianView} g
       * @returns {string}
       */
      function renderGuardianBuildEnvOverrides(g) {
        const inherited = g.combined_env || {};
        const own = g.build_env_overrides || {};
        const apiPath = `/api/guardians/${g.id}/build-env`;
        const keys = Array.from(new Set(Object.keys(inherited).concat(Object.keys(own)))).sort();
        const overrideCount = Object.keys(own).length;
        const banner = overrideCount
          ? `<div class="kv-row" style="margin:0 0 2px" data-tip="The finalize-time build/check-gate step does not run with exactly the environment the combined branch stack was built under.">⚠ ${overrideCount} build override${overrideCount === 1 ? "" : "s"} active</div>`
          : "";
        const rows = keys.length
          ? keys.map((k) => renderInheritedEnvOverrideRow(k, inherited, own, apiPath, "the build step")).join("")
          : `<div class="kv-row" style="margin:0"><span class="v">None</span></div>`;
        return `<h3 class="section" style="margin-top:8px" data-tip="The environment the finalize-time build/check-gate step runs under (RAL-203).\nBy default this is the last enabled branch's own resolved environment — the same variables the code was written under.\nOverride or unset any of them here to change just this step, independent of the manual-checks step below.">environment overrides (build step)</h3>
          ${banner}
          ${rows}
          <div class="kv-row" style="margin:2px 0 0">${envViewerBtn(apiPath, "the auto-build step")}<button class="btn" data-click="addEnvOverrideAt" data-api-path="${esc(apiPath)}" data-tip="Add an environment variable for the build/check-gate step only.\nWho/when: the check gate needs a variable no branch set — a CI-only flag, a different API base, a test credential.\nApplies the next time this review's check gates run, and stays set until removed.">+ Add override</button></div>`;
      }
      /**
       * Renders the manual-checks step's "environment overrides" section
       * (RAL-203) — the LLM-suggested commands run via `ralphus review checks
       * run` / the board's "Run all". Shadows the same `g.combined_env`
       * baseline as `renderGuardianBuildEnvOverrides`, but as its own,
       * independent layer.
       * @param {GuardianView} g
       * @returns {string}
       */
      function renderGuardianManualChecksEnvOverrides(g) {
        const inherited = g.combined_env || {};
        const own = g.manual_checks_env_overrides || {};
        const apiPath = `/api/guardians/${g.id}/manual-checks-env`;
        const keys = Array.from(new Set(Object.keys(inherited).concat(Object.keys(own)))).sort();
        const overrideCount = Object.keys(own).length;
        const banner = overrideCount
          ? `<div class="kv-row" style="margin:0 0 2px" data-tip="The manual-checks step (\"Run all\" and individual check commands) does not run with exactly the environment the combined branch stack was built under.">⚠ ${overrideCount} manual-checks override${overrideCount === 1 ? "" : "s"} active</div>`
          : "";
        const rows = keys.length
          ? keys.map((k) => renderInheritedEnvOverrideRow(k, inherited, own, apiPath, "the manual-checks step")).join("")
          : `<div class="kv-row" style="margin:0"><span class="v">None</span></div>`;
        return `<h3 class="section" style="margin-top:8px" data-tip="The environment the manual-checks step runs under (RAL-203) — \"Run all\", an individual suggested command, or a [[review.action]] hint.\nBy default this is the last enabled branch's own resolved environment.\nOverride or unset any of them here to change just this step, independent of the build step above.">environment overrides (manual checks)</h3>
          ${banner}
          ${rows}
          <div class="kv-row" style="margin:2px 0 0">${envViewerBtn(apiPath, "the manual-checks step")}<button class="btn" data-click="addEnvOverrideAt" data-api-path="${esc(apiPath)}" data-tip="Add an environment variable for the manual-checks step only.\nWho/when: a suggested command needs a variable no branch set — a reviewer-only flag, a different API base, a test credential.\nApplies the next time a manual check runs, and stays set until removed.">+ Add override</button></div>`;
      }
      /**
       * Prompts for a value and sets it as this scope's own override for
       * `key` (RAL-191/RAL-203), shadowing the inherited value for this scope
       * only. Shared by review-branch, build-step, and manual-checks-step env
       * sections — `apiPath` fully addresses which layer is mutated.
       * @param {string} apiPath
       * @param {string} key
       * @param {string} scopeLabel
       * @returns {Promise<void>}
       */
      async function setScopedEnvOverride(apiPath, key, scopeLabel) {
        const value = prompt(`Value for ${key} in ${scopeLabel}:`);
      ...
      /**
       * Tombstones `key` for this scope (RAL-191/RAL-203) — the variable is
       * removed from this scope's environment even though it would otherwise
       * be inherited.
       * @param {string} apiPath
       * @param {string} key
       * @param {string} scopeLabel
       * @returns {Promise<void>}
       */
      async function unsetScopedEnvOverride(apiPath, key, scopeLabel) {
        if (!confirm(`Remove "${key}" from ${scopeLabel}'s environment?\nIt will not be set at all here, even though it would otherwise be inherited.\nThis cannot be undone.`)) return;
        const res = await post(apiPath, { unset: [key] });
        if (!res.ok) { alert("Failed to unset the environment variable."); return; }
        tick();
      }
      /**
       * Drops this scope's own entry for `key` (RAL-191/RAL-203), so it goes
       * back to inheriting the upstream value.
       * @param {string} apiPath
       * @param {string} key
       * @param {string} scopeLabel
       * @returns {Promise<void>}
       */
      async function clearScopedEnvOverride(apiPath, key, scopeLabel) {
        if (!confirm(`Go back to inheriting "${key}" for ${scopeLabel}?\nThis cannot be undone.`)) return;
        const res = await post(apiPath, { clear: [key] });
        if (!res.ok) { alert("Failed to revert the environment override."); return; }
        tick();
      }
      /**
