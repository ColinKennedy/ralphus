      ...
      function projectFilterMenuRowsHtml() {
        ...
        return names.map((name) => `<div class="ctx-check ${filters.projects.has(name) ? "on" : ""}"><label style="display:flex;align-items:center;gap:6px;width:100%;margin:0;cursor:pointer" onclick="event.stopPropagation()"><input type="checkbox" ${filters.projects.has(name) ? "checked" : ""} onchange="toggleProjectFilter('${esc(name)}',this.checked)">${esc(name)}</label></div>`).join("");
      }
      /**
