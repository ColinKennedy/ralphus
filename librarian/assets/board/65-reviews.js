...
Check the task's cell output and re-run it — or, if this branch is meant to be empty, disable it to drop it from the stack.">⌀ empty</span>`;
      ...
      // ---------- worktree retirement view (RAL-385) ----------
      ...
      const RETIREMENT_STATES = ["scheduled", "eligible", "claimed", "failed", "retired"];
      /** @type {{[state: string]: string}} */
      const RETIREMENT_STATE_COLORS = { scheduled:"--pending", eligible:"--queued", claimed:"--ignored", failed:"--failed", retired:"--cancelled" };
      /** @type {WorktreeRetirementEntry[]} rows from the open modal's last poll */
      let retirementEntries = [];
      /** @type {string[]} active state filter chips (empty = every state) */
      let retirementStateFilters = [];
      /** @type {string} message from the last failed fetch in the retirement modal */
      let retirementModalError = "";
      /**
       * Polls `/api/worktree-retirements` for the open retirement modal.
       * @returns {Promise<void>}
       */
       ...
       * like queued work, claimed=amber caution, failed=red, retired=grey).
       * @param {string} s
       * @returns {string}
       */
      ...
      /**
       * Toggles one state filter chip in the retirement modal and re-renders.
       * @param {string} s
       * @returns {void}
       */
      function toggleRetirementState(s) {
        ...
        renderRetirementModal();
      }
      /**
       * Refreshes the retirement rows and re-renders the open modal.
       * @returns {Promise<void>}
       */
      async function refreshWorktreeRetirements() {
        await pollWorktreeRetirements();
        renderRetirementModal();
      }
      /**
       * Opens the Reviews-tab "worktree retirement" popup (RAL-385): the
       * operator's view of every review worktree across retirement states,
       * with per-state filter chips and failure context for cleanups that
       * need intervention.
       * @returns {Promise<void>}
       */
      async function openWorktreeRetirementModal() {
        retirementStateFilters = [];
        retirementModalError = "";
        await pollWorktreeRetirements();
        renderRetirementModal();
      }
      /**
       * Closes the worktree-retirement popup.
       * @returns {void}
       */
      function closeWorktreeRetirementModal() {
        closeModal();
      }
      /**
       * Renders the open worktree-retirement popup: filter chips over
       * RETIREMENT_STATES, then one row per entry matching the active
       * filters -- state badge, review, worktree path, eligible-at (or last
       * attempt) time, and a detail column carrying the blocking claim or
       * the failure error.
       * @returns {void}
       */
      function renderRetirementModal() {
        ...
        const rowsHtml = rows.length
          ? rows.map((e) => {
              ...
              const detail = e.error
                ? `<span style="color:var(--failed)" data-tip="The last cleanup attempt failed with this git error. The daily sweep retries automatically; keep an eye on it or investigate manually.">${esc(e.error)}</span>`
                : e.claim_kind
                ...
                : `<span style="color:var(--muted)" data-tip="When the worktree becomes old enough to retire. 'eligible' means the daily sweep will remove it on its next pass; 'scheduled' means it is not old enough yet.">${esc(new Date(e.eligible_at_ms).toLocaleString([], { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" }))}</span>`;
              return `<tr>
        ...
        const err = retirementModalError ? `<div class="verr" style="margin-top:8px">${esc(retirementModalError)}</div>` : "";
        byId("modal-root").innerHTML = `
          <div class="modal-bg" onclick="if(event.target===this)closeWorktreeRetirementModal()"><div class="modal" style="width:760px;max-width:96vw">
            <h2>Worktree retirement</h2>
            <div class="k" style="margin-bottom:8px" data-tip="Review worktrees idle for 30 days are retired by a daily daemon sweep. scheduled = too young; eligible = the next sweep removes it; claimed = kept because a live task/proof/review still needs it; failed = the last attempt failed; retired = removed (kept here as history for as long as the review exists).">Every review worktree by retirement state — what the daily sweep will remove, what it kept, and what failed.</div>
            ...
            <table class="proj-table"><thead><tr>
              ...
              <th data-tip="scheduled/eligible: when the worktree becomes (or became) eligible. claimed: the claim holding it. failed: why the attempt failed. retired: when it was removed.">Detail</th>
            </tr></thead><tbody>${rowsHtml}</tbody></table>
            ${err}
            <div class="btn-row">
              <button class="btn" onclick="refreshWorktreeRetirements()" data-tip="Re-fetch the retirement view from the daemon.">↻ Refresh</button>
              <button class="btn" onclick="closeWorktreeRetirementModal()" data-tip="Close this popup.">Close</button>
            </div>
          </div></div>`;
      }
