...
/// The tracer name every librarian span is created under, and the OTLP
/// instrumentation scope name spans are reported with.
const TRACER_NAME: &str = "ralphus-librarian";
...
pub fn init(service_name: &'static str) -> Option<SdkTracerProvider> {
    ...
    eprintln!("ralphus-librarian [otel] exporting traces to {endpoint}");
    Some(provider)
}

/// Flush and shut down a tracer provider returned by [`init`]. Best-effort —
/// shutdown errors are logged to stderr, never stdout.
pub fn shutdown(provider: Option<SdkTracerProvider>) {
    if let Some(provider) = provider {
        if let Err(e) = provider.shutdown() {
            eprintln!("ralphus-librarian [otel] shutdown: {e}");
        }
    }
}

/// The librarian's tracer, fetched through the global API per RAL-96's
...
/// propagate `active.cx` across a process boundary via
...
impl ActiveSpan {
    ...
    /// Attach an attribute (e.g. `http.method`) to the span.
...
/// Start a span named `name` as a child of `parent`, using the librarian's
...
/// for forwarding across the next hop (an outbound HTTP call to the daemon).
