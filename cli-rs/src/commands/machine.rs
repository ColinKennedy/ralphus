...
pub fn parse(args: &[String]) -> MachineCommand {
    ...
    match args.first().map(String::as_str) {
        ...
        Some("cleanup") => match scanner.remaining().into_iter().next() {
            Some(machine) => MachineCommand::Cleanup { machine },
            None => MachineCommand::UsageError("cleanup requires a <machine> argument".to_string()),
        },
...
pub fn dispatch(cmd: MachineCommand, opts: &GlobalOpts) -> i32 {
    ...
    match cmd {
        ...
        MachineCommand::Cleanup { machine } => match client.cleanup_machine(&machine) {
            Ok(_) => {
                println!("cleaned up workspace \"{machine}\"");
                0
            }
            Err(e) => {
                CommandError::Daemon(e).print(false, None);
...
mod tests {
    ...
    fn parses_get_remove_cleanup() {
        ...
        match parse(&v(&["cleanup", "ssh:host1"])) {
            MachineCommand::Cleanup { machine } => assert_eq!(machine, "ssh:host1"),
            other => panic!("unexpected: {other:?}"),
        }
    }
