...
fn validate_raw(raw: &toml::Table) -> Vec<String> {
    ...
    const KNOWN_TOP: [&str; 5] = ["task", "daemon", "review", "defaults", "ark"];
    for key in raw.keys() {
...
mod tests {
    ...
    #[test]
    fn validate_raw_flags_agent_section_as_unknown() {
        // RAL-460: agent profiles moved into the daemon's own store --
        // `[agent.profiles.*]` is no longer a recognized TOML section at
        // all, so it now falls through to the generic unknown-top-level-key
        // check like any other stray key would.
...
"#
        ...
        let issues = validate_raw(&raw);
        assert!(
            issues.iter().any(|i| i.contains("\"agent\" is unknown")),
            "{issues:?}"
        );
    }
