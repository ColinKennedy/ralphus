...
mod tests {
    ...
    #[test]
    fn reviewer_system_prompt_checks_the_review_mailbox_every_turn() {
        // RAL-375: Reviewer must drain PR/CI-watch notices, but only its own
        // "review" category -- never the unfiltered watcher drain.
        let content = reviewer_system_prompt_content(None, false, CLAUDE_READ_ONLY_MECHANISM);
        let program = crate::program_name::resolve_program_name();
        assert!(content.contains("MANDATORY"));
        assert!(content.contains("after every user turn"));
        assert!(content.contains(&format!("{program} mailbox check --category review")));
    }
    ...
    fn watcher_system_prompt_drains_review_category_messages_too() {
        ...
        // PR/CI-watch notices -- so it never adds a `--category` filter.
        let content = watcher_system_prompt_content(false, CLAUDE_READ_ONLY_MECHANISM);
