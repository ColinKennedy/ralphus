...
pub enum CellCommand {
    ...
    /// RAL-324: read-only listing of this cell's resolved environment.
    /// `scope` is `cell` (what the cell's own subprocess runs with) or
    /// `proof` (what its proof steps inherit).
    Env {
        selector: String,
        scope: String,
    },
...
pub fn parse(args: &[String]) -> CellCommand {
    ...
    match args.first().map(String::as_str) {
        ...
        Some("env") => {
            match crate::commands::env::take_scope(&mut scanner, "cell", &["cell", "proof"]) {
                Ok(scope) => {
                    with_selector(scanner, |selector| CellCommand::Env { selector, scope })
                }
                Err(message) => CellCommand::UsageError(message),
            }
        }
        ...
        Some("edit") => {
            ...
            let auto_compact_threshold = scanner
        ...
        Some("remote-terminal") => {
            with_selector(scanner, |selector| CellCommand::RemoteTerminal { selector })
        }
        Some("open-agent") => {
...
/// Builds the local CLI command that resumes `agent_session_id`, delegating
/// to [`ralphus_core::agent_resume::agent_resume_argv`] (shared with
/// `cli/src/commands/review.rs` and, for the daemon's own spawn path,
/// `daemon/src/server.rs::open_agent_terminal`) so this printed command
/// matches what "Open Agent" itself would actually run.
///
/// `claude_program`/`codex_program`/`pi_program` are resolved from the
/// `RALPHUS_CLAUDE_COMMAND`/`RALPHUS_CODEX_COMMAND`/`RALPHUS_PI_COMMAND` env
/// overrides here, falling back to the bare agent name -- RAL-468: this used
/// to hardcode the literal names, silently ignoring the same override the
/// daemon's real spawn already respected.
#[must_use]
...
pub fn dispatch(cmd: CellCommand, opts: &GlobalOpts) -> i32 {
    ...
    match cmd {
        ...
        CellCommand::Env { selector, scope } => run_and_report(opts, None, || {
            let resolved = resolve_scoped(&client, &selector, "cell")?;
            let view = client.env_view(&crate::commands::env::cell_path(&resolved, &scope))?;
        ...
        CellCommand::RemoteTerminal { selector } => run_and_report(opts, None, || {
            let resolved = resolve_scoped(&client, &selector, "cell")?;
            crate::terminal_relay::run(
                &client,
                &resolved.squad_id,
                resolved.task_idx,
                resolved.cell_idx,
            )
            .map_err(|e| CommandError::Selector(SelectorError(e)))
        }),
...
fn render_cell_detail(s: &Value) {
    ...
    crate::output::print_kv(&[
        ...
        // RAL-326: shown as their own rows, not folded into `tokens_in` --
        // for an agentic session these are routinely the bulk of billed
        // input, and a backend that reports no cache breakdown leaves both 0.
        (
...
mod tests {
    ...
    fn parses_env_with_a_default_and_an_explicit_scope() {
        match parse(&v(&["env", "squad-1/build/worker"])) {
            CellCommand::Env { selector, scope } => {
                assert_eq!(selector, "squad-1/build/worker");
                assert_eq!(scope, "cell");
            }
            other => panic!("unexpected: {other:?}"),
        }
        match parse(&v(&["env", "squad-1/build/worker", "--scope", "proof"])) {
            CellCommand::Env { scope, .. } => assert_eq!(scope, "proof"),
            other => panic!("unexpected: {other:?}"),
        }
        matches!(
            parse(&v(&["env", "squad-1/build/worker", "--scope", "task"])),
    ...
    #[test]
    fn parses_edit_with_system_prompt() {
        match parse(&v(&[
            "edit",
            "squad-1/build/0",
            "--system-prompt",
    ...
    #[test]
    fn parses_remote_terminal() {
        match parse(&v(&["remote-terminal", "squad-1/build/0"])) {
            CellCommand::RemoteTerminal { selector } => {
                assert_eq!(selector, "squad-1/build/0");
            }
            other => panic!("unexpected: {other:?}"),
        }
    ...
    #[test]
    fn remote_terminal_requires_a_selector() {
        assert!(matches!(
            parse(&v(&["remote-terminal"])),
            CellCommand::UsageError(_)
        ));
    }
