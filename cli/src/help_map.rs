...
//! order) -- `[type]` is `str`/`integer`/`float`/`path`/`uri`, with `, optional`
...
/// Renders entity-addressing arguments with their distinct `[uri]` type while
/// keeping the command registry's parser-oriented chips unchanged.
...
/// The guidance notes, blank-line separated, followed by [`generate()`]'s
