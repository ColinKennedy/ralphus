...
- ralphus --daemon-url [url] --json --version  {Submit and manage autonomous agent tasks against the ralphus daemon.}
    ...
    - (read-only-safe) graph squad_id [uri, optional] --all --dot  {Render the task-order dependency graph. (Rust port simplifies Python's --global/--format ascii|dot choice to plain --dot/--all boolean flags.)}
    ...
    - retry squad_id [uri]  {Re-run a squad from scratch (reset to pending). (Rust port: squad-level only; Python's per-selector --environment/--env-file overrides are not yet ported.)}
    - review (subagent)  {Inspect and act on reviews (guardians).}
        ...
        - feedback selector [uri] text [str]  {Post feedback on one branch, triggering a resolver re-attempt.}
        ...
        - pr  {Submit/query pull requests for a review.}
            - (read-only-safe) comments pr_id [uri]  {List a PR's comments/notes.}
            ...
            - pull-feedback pr_id [uri]  {Action a PR's un-actioned feedback into the owning review worktree.}
            - pull-from-pr pr_id [uri]  {Pull a reviewer's commits pushed directly to the PR branch back into the owning review worktree, resolving conflicts and restacking downstream branches (RAL-190).}
            - (read-only-safe) show pr_id [uri]  {Show one PR row.}
            - submit selector [uri] --alias [name] --combined --description [text] --position [integer] --title [text] --use-worktree-branch-name  {Submit a PR/MR for one stacked branch or the combined worktree.}
            ...
            - update pr_id [uri] --branch-alias [name] --pr-number [integer] --pr-url [url] --state [open|merged|closed]  {Mutate the recorded PR mapping, e.g. after a PR is closed and reopened under a new number.}
        ...
        - settings selector [uri] --auto-pr-feedback/--no-auto-pr-feedback --auto-submit-pr-stack/--no-auto-submit-pr-stack --base-branch [branch] --match-pr-branch-name/--no-match-pr-branch-name --proof-scope [each_branch|final_branch|nothing] --resolver-agent [name] --resolver-model [name] --skip-auto-build/--no-skip-auto-build --skip-auto-clean/--no-skip-auto-clean --skip-base-updates/--no-skip-base-updates --skip-worktrees/--no-skip-worktrees  {Update per-review opt-out settings.}
    ...
    - squad  {Inspect and act on squads.}
        - activate squad_id [uri]  {Promote a held (queued) squad to pending.}
        - cancel squad_id [uri]  {Cancel a squad.}
        - delete squad_id [uri] --yes  {Permanently delete a squad.}
        - edit squad_id [uri] --label [text]  {Edit a squad's fields.}
        - (read-only-safe) env squad_id [uri]  {List a squad's resolved environment variables, read-only (RAL-324); values of names registered in the Secrets tab are masked.}
        - (read-only-safe) list --name [substring] --sort [date|name] --status [states]  {List squads.}
        - (read-only-safe) logs squad_id [uri]  {Show a squad's state-transition audit log.}
        - rename squad_id [uri] label [str]  {Rename a squad's label.}
        - restart squad_id [uri]  {Restart a whole squad, dirtying every squad that depends on it.}
        - retry squad_id [uri]  {Re-run with the same parameters (reset to pending).}
        - set-status squad_id [uri] state [str]  {Manually override a squad's status.}
        - (read-only-safe) show squad_id [uri]  {Show a single squad's detail.}
        - timeline squad_id [uri] --write [path]  {Generate the merged, chronological uber-log-viewer timeline for a squad (RAL-155).}
    - (read-only-safe) status squad_id [uri, optional] --concurrency  {Show squad status from the daemon.}
