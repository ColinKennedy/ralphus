...
(`cli/src/selector.rs`). Anywhere this reference writes `<selector>` — and for
`review link-cell`'s `<cell_selector>` and `review move-branch`'s `<to_review>`,
which resolve through the same selectors — both the **ralphus URI** form below
and the legacy selector form are accepted. So are the queue item paths of
`queue set-status`/`reorder`/`set-position`.

A plain `<squad_id>` positional (the `squad *` family, `status`, `graph`) is the
**exact opaque id only** (`squad-000000000001`): the CLI passes it straight to
`/api/squads/{id}`, so labels, names, indices, and URI forms are **not** accepted
there — the help map labels such arguments `[id]` to say so.
