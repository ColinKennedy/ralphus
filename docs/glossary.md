...
| **preset** | A daemon-registered, named bundle of field defaults (`daemon::presets`; `ralphus preset register/list/get/deregister`, the board's Presets tab): `system_prompt`, `system_prompt_position` (cell-only), `maximum_context`, `auto_compact_threshold` (task/cell only), and `maximum_tool_output_tokens` (task/cell/proof). Referenced from a task's, cell's, or proof step's own `extends` array as a wrapped `<<ralphus:presets/<name>>>` **sentinel**; at submit time the daemon stamps each named preset's field values into any of that *same entity's* own fields still unset — never overriding an explicitly-typed value, and silently skipping a field that doesn't apply to the entity kind it's referenced from. When an `extends` list names more than one preset defining the same field, the last one listed wins. Global only (no per-project override), store-backed, mutable via CLI/MCP/board like a Triage type — not config-file-only like an **agent profile**. Once stamped, the resulting value carries no provenance and is indistinguishable from one the author typed directly. |
