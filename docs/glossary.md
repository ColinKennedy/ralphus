...
| **worktree retirement** | The daily daemon sweep that removes review worktrees idle past a 30-day threshold (`WORKTREE_RETIREMENT_AGE_MS`, `guardian_merge::retire_stale_worktrees`), keeping any held by a non-terminal claim (which raises one durable mailbox escalation instead). Each worktree in the view (`GET /api/worktree-retirements`, `ralphus review worktree-retirements`, the board's Reviews-tab ⏳ Retirement popup) is in exactly one state: **scheduled** (too young — `eligible_at_ms` says when it crosses the threshold), **eligible** (old enough, awaiting the next daily sweep), **claimed** (old enough but held by a live task/proof/review claim), **failed** (an attempt ran and git refused it — the `error` stays visible until a retry succeeds), or **retired** (removed). "Eligible" deliberately avoids saying **stale** (already taken by Ark) and "retired" deliberately avoids **done/deployed** (taken by review and squad lifecycle states). Retired history is durable in `guardian_worktree_retirements` and lives exactly as long as its review's row, since a successful retirement clears the persisted path columns that would otherwise be the only trace. |
