...
| **merged** | Three same-spelled, differently-scoped meanings — say which one you mean: (1) the whole **review**/**guardian**'s own terminal `GuardianStatus` (RAL-479; was `approved`) — set once every linked PR/MR has merged, or once the base has absorbed every enabled branch; (2) a `PullRequestView.state` value — one PR/MR row's own forge lifecycle state; (3) a per-**branch** `MergeStatus` (RAL-480) — that one branch's own commits are already integrated upstream, detected by git ancestry or by its own PR/MR's forge state, independent of the review's status or any other branch's. A review can carry a mix of `merged` and not-yet-merged branches (a partial stack merge) and still sit `in_review`; a `merged` branch stays enabled and keeps rebasing normally, it just never gets its PR/MR created, updated, or otherwise touched again. |
...
| **fork** (RAL-338) | The writable repository a project's work and review branches are pushed to for one submitting user. Registered per `(project, user)` (`daemon/src/project_forks.rs`); if that user has no row, ralphus uses the project's origin. Ralphus only *registers* an existing fork; it never creates one through a forge API. Do not use "upstream" or "origin" for this — both are already taken (see **base branch**'s `upstream` entry above, and the git remote convention). |
...
| **dual root PR** (`dual_root_pr`) | Opt-in (off by default; project- and per-review-settable, same precedence chain as **PR branch**'s `separate_pr_branch`): a fork-mode review's root branch — and whichever branch **promotion** later re-files against the **parent** — also gets a second, fork-internal "stack" PR into the review's own transient fork branch (`ralphus/review/<review id>/upstream`, collision-suffixed against the fork's existing heads), so it visually joins the rest of the stack instead of standing apart from it. Ralphus force-pushes that transient branch to the **parent**'s current base-branch tip before submission and promotion, and again on every later fetch of the parent branch for the review; it never touches the fork's own copy of the base branch, and no PR ever targets it. On a review with no fork in use the setting is a logged no-op. Its real cross-repository PR against the **parent** (the one that actually merges) is unaffected. When that PR merges, the stack PR is closed with a pointer comment, never merged; the branch **promotion** re-files gets its own fresh stack PR the same way a freshly-submitted root branch would. The transient branch is deleted from the fork once the review reaches a terminal state or is deleted. |
| **superseded** (CI run/pipeline, RAL-510) | A GitHub Actions run or GitLab pipeline still queued/in-progress on a **PR branch** after that branch was force-pushed to a new tip — its tree no longer exists on the branch, so its eventual result is meaningless. Not the same sense as **promotion**'s "superseded PR" above (a forge PR row replaced by a new one after a cross-repository re-file); this sense is about CI *runs*, not PR *rows*, and the PR/MR itself is untouched. `ForgeClient::cancel_superseded_ci(branch, keep_sha)` (`daemon/src/forge.rs`) force-cancels every active run/pipeline on a branch except the one matching `keep_sha` — the tip that was just pushed — after each of the daemon's force-push call sites, gated by the per-review `auto_cancel_outdated_pr_pipelines` setting (on by default). Purely a runner-cost optimization: a cancelled superseded run reads as `cancelled`, but only if its own head SHA is still current, and ralphus's CI-status classification is already scoped to the PR's current head SHA (see `forge.rs`'s check-runs/pipeline-SHA-staleness logic), so a superseded run's cancellation can never flip a still-open PR's `ci_status` or trigger `auto_fix_pr_errors`. |
...
| **linked field** (RAL-460) | An `environment` value of the form `<<ralphus:linked-field/<path>[?text=<function>({})]>>`, where `<path>` is a relative-path-shaped address (`.` = the table this value is declared on, `..` = one level up in TOML nesting) ending in a target field name — `cwd`/`id`, or `environment.<key>` for a same-table sibling. Resolves once the target field has a real value (a materialized worktree `cwd`, or a plain literal read directly). May chain (a linked field pointing at another linked field); the daemon resolves the chain in dependency order. The optional `?text=<function>({})` query (RAL-460 follow-up) applies a registered text-transform function — today only `basename` — to the resolved value before it's used. `[[default]].environment` may also set one, applying to every task/cell/proof step in the submission (RAL-460 follow-up). See `docs/special-syntax.md`. |
...
| **prophecy** | A durable, append-only record of what an agent — or ralphus itself, e.g. a `guardian_merge.rs` rebase/conflict-resolution decision — learned mid-work, keyed by `entity_uri` + `attempt` (`daemon/src/prophecy.rs`, `ralphus prophecy list`/`show`). Distinct from a **ghost** (one merge-on-write note per owner, cascade-deleted with its squad) in that a prophecy is many rows, never overwritten, and survives its squad's deletion; distinct from **Cartographer** (system-wide event log, pruned at 30 days/50k rows) in that a prophecy is meant to be read by a human in the PR, not just queried for diagnostics — though every prophecy write also emits one Cartographer row. See `docs/prophecy-design.md`. |
