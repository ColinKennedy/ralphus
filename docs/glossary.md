...
| **dual root PR** (`dual_root_pr`) | Opt-in (off by default; project- and per-review-settable, same precedence chain as **PR branch**'s `separate_pr_branch`): a fork-mode review's root branch — and whichever branch **promotion** later re-files against the **parent** — also gets a second, fork-internal "stack" PR into a **mirror branch**, so it visually joins the rest of the stack instead of standing apart from it. Its real cross-repository PR against the **parent** (the one that actually merges) is unaffected. When that PR merges, the stack PR is closed with a pointer comment, never merged; the branch **promotion** re-files gets its own fresh stack PR the same way a freshly-submitted root branch would. |
| **mirror branch** | The fork-side branch (`ralphus-mirror/<base branch>/<guardian id>`, scoped per-review so concurrent reviews sharing one **fork** never race to force-push the same ref) a **dual root PR**'s stack PR targets — force-pushed to the **parent**'s current base-branch tip before every submission and promotion, never itself merged. A fork whose CI runs on every branch push should exclude `ralphus-mirror/**` from it; ralphus has no way to suppress a fork owner's own CI config, and every sync is a real push. |
