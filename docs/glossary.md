...
| **linked field** (RAL-460) | An `environment` value of the form `<<ralphus:link/<field>>>` (optionally `?suffix=<relative-path>`) that resolves to another field on the *same* cell — `cwd`, or `environment.<key>` for a sibling entry — instead of re-embedding that field's placeholder text. Resolves once the target field has a real value (a materialized worktree `cwd`, or a plain literal read directly). May chain (a linked field pointing at another linked field); the daemon resolves the chain in dependency order. See `docs/special-syntax.md`. |
