...
| **agent profile** | RAL-460: a named row in the daemon's own `agent_profiles` store (managed via the board's admin-only Agent Profiles tab, or `ralphus agent profile ...`) that resolves a task/cell `agent = "<name>"` to a concrete backend, optional default model/executable override, and its own environment variables (each literal or a link to another env var, resolved at cell-run time). Global across every project — no per-project TOML layering. `"claude-code"` and `"codex"` are always-present, **locked** rows: their name/backend can never change and they can never be deleted, only `executable`. There is no TOML syntax for this anymore. |
