...
| Node.js 22+ | pinned in CI (`.github/workflows/ci.yml`) | Lints/type-checks/tests `librarian/assets/board.html`'s inline JS (`npm run lint` / `typecheck` / `knip` / `test`) | Dev-only — see `package.json`'s own description: "Not a runtime dependency — the librarian embeds board.html via `include_str!` and ships no Node code." No `.nvmrc` at repo root. |
