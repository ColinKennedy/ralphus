...
pub const CATALOG: &[CatalogEntry] = &[
    ...
    CatalogEntry {
        ...
        cost_tier: CostTier::Free,
        requirement: RequirementLevel::Optional,
        probe: Probe::DaemonApi("GET /api/health/report"),
        impact: "Tasks using Claude Code cannot start if the effective command (a database override, $RALPHUS_CLAUDE_COMMAND, or the default claude program) is unavailable.",
        remediation: "Install claude on PATH, or set a working override (database or $RALPHUS_CLAUDE_COMMAND).",
    },
    CatalogEntry {
        id: ID_CODEX_COMMAND,
        label: "codex command",
        ...
        requirement: RequirementLevel::Optional,
        probe: Probe::DaemonApi("GET /api/health/report"),
        impact: "Tasks using Codex cannot start if the effective command (a database override, $RALPHUS_CODEX_COMMAND, or the default codex program) is unavailable.",
        remediation: "Install codex on PATH, or set a working override (database or $RALPHUS_CODEX_COMMAND).",
    },
    CatalogEntry {
        id: ID_PI_COMMAND,
        label: "Pi harness command",
        section: section::HARNESS,
        applicability: Applicability::DaemonLocal,
        cost_tier: CostTier::Free,
        requirement: RequirementLevel::Optional,
        probe: Probe::DaemonApi("GET /api/health/report"),
        impact: "Tasks using Pi cannot start if the effective command (a database override, $RALPHUS_PI_COMMAND, or the default pi program) is unavailable or older than 0.85.1.",
        remediation: "Install Pi 0.85.1 or later on PATH, or set a working override (database or $RALPHUS_PI_COMMAND) to a compatible executable.",
    },
    CatalogEntry {
        id: ID_CONFIG,
